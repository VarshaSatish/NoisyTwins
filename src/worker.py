...
from utils.barlow import BarlowTwins, compute_contrastive
...
class WORKER(object):
    def __init__(self, cfgs, run_name, Gen, Gen_mapping, Gen_synthesis, Dis, Gen_ema, Gen_ema_mapping, Gen_ema_synthesis,
                 ...
                 best_step, best_fid, best_ckpt_path, lecam_emas, loss_list_dict, metric_dict_during_train, effective_num_samples_lambda) :
        self.cfgs = cfgs
        ...
        self.lam = [1] * cfgs.DATA.num_classes
        ...
        if lecam_emas is not None:
            self.lecam_ema.__dict__ = lecam_emas
        ...
        if self.LOSS.apply_noisytwins:
            self.noisytwins = BarlowTwins(self.MODEL.w_dim, lambd=self.LOSS.noisytwins_momentum_lambda, 
                                        use_projector=False).to(self.local_rank)
            self.barlow_loss = compute_contrastive

        if self.DATA.name == "CIFAR10":
            self.num_eval = {"train": 50000, "test": 10000}
        elif self.DATA.name == "CIFAR10_LT":
            self.num_eval = {"train": 50000, "test": 10000, "ifid_num": 5000}
        ...
        elif self.DATA.name == "iNat19":
            self.num_eval = {"train": 50000, "valid": 10000, "test": 10000, "ifid_num": 5000}
        elif self.DATA.name in ["Imgnet_carniv", "AnimalFace_FS"]:
            self.num_eval = {"train": 10000, "valid": 10000, "ifid_num": 5000}
        elif self.DATA.name == "imagenet_lt":
            self.num_eval = {"train": 50000, "valid": 10000, "test": 10000, "ifid_num": 5000}
        else:
        ...
        for step_index in range(self.OPTIMIZATION.d_updates_per_step):
            ...
            for acml_index in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision and not self.is_stylegan else misc.dummy_context_mgr() as mpc:
                    ...
                    if self.MODEL.bool_plot_emb_norm: # Added for plotting emb norm
                        with torch.no_grad():
                            if isinstance(self.Dis, DataParallel) or isinstance(self.Dis, DistributedDataParallel):
                                self.dis_emb_norms = torch.norm(self.Dis.module.mapping.embed.weight.T, dim=1)
                            else:
                                self.dis_emb_norms = torch.norm(self.Dis.mapping.embed.weight.T, dim=1)
                    else:
                        self.dis_emb_norms = None
                    ...
                    real_dict = self.Dis(real_images_, real_labels, is_real=True)
                    ...
                    if self.AUG.apply_ada or self.AUG.apply_apa or self.AUG.apply_diffaug:
                    ...
                    if self.LOSS.apply_cr:
                        ...
                        real_prl_dict = self.Dis(real_prl_images, real_labels, is_real=True)
                    ...
                    if self.LOSS.apply_lecam:
                        ...
                        if current_step > self.LOSS.lecam_ema_start_iter:
                            self.lecam_loss = losses.lecam_reg(real_adv_output, fake_adv_output, self.lecam_ema)
                        else:
                            self.lecam_loss = torch.tensor(0., device=self.local_rank)
                        dis_acml_loss += self.LOSS.lecam_lambda*self.lecam_loss
                    ...
                    if self.LOSS.apply_r1_reg and not self.is_stylegan:
                        ...
                        real_images.requires_grad_(True)
                        real_dict = self.Dis(self.AUG.series_augment(real_images), real_labels, is_real=True)
        ...
        for step_index in range(self.OPTIMIZATION.g_updates_per_step):
            ...
            for acml_step in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision and not self.is_stylegan else misc.dummy_context_mgr() as mpc:
                    # sample fake images and labels from p(G(z), y)
                    if self.LOSS.apply_noisytwins:
                        y_sampler = "totally_random_barlow"
                    else:
                        y_sampler = "totally_random"
                    ...
                    if self.MODEL.bool_plot_emb_norm: # Added for plotting emb norm
                        with torch.no_grad():
                            self.gen_emb_norms = torch.norm(self.Gen.mapping.embed.weight.T, dim=1)
                    else:
                        self.gen_emb_norms = None
                    ...
                    # applying the NoisyTwins regulariser using BarlowTwins.
                    # Divide the batch in create the twins in w-space, which are originally 
                    # created using same z and different noise augmented c, having same labels for both twins.
                    # (See sample.py).
                    if self.LOSS.apply_noisytwins:    
                        if current_step >= self.LOSS.noisytwins_start_iter:
                            reduced_bs = self.OPTIMIZATION.batch_size//2
                            self.noisytwins_loss = self.barlow_loss(ws[:reduced_bs, 0, :], ws[reduced_bs:, 0, :], self.noisytwins, self.LOSS.noisytwins_lambda)
                        else:
                            self.noisytwins_loss = torch.tensor(0., device=self.local_rank)
                        gen_acml_loss += self.noisytwins_loss
            ...
            if self.STYLEGAN.apply_pl_reg and (self.OPTIMIZATION.g_updates_per_step*current_step + step_index) % self.STYLEGAN.g_reg_interval == 0:
                if current_step >= self.STYLEGAN.pl_start_iter:
                    self.OPTIMIZATION.g_optimizer.zero_grad()
                    for acml_index in range(self.OPTIMIZATION.acml_steps):
                        fake_images, fake_labels, fake_images_eps, trsp_cost, ws, _, _ = sample.generate_images(
                            z_prior=self.MODEL.z_prior,
                    ...
                    self.pl_reg_loss = torch.tensor(0.0, device=self.local_rank)
    ...
    def log_train_statistics(self, current_step, real_cond_loss, gen_acml_loss, dis_acml_loss):
        ...
        if self.MODEL.bool_plot_emb_norm:
            keys = range(0, self.DATA.num_classes)
            dct = {}
            for k in keys:
                dct[f'Class {k+1}'] = self.dis_emb_norms[k].item()
            wandb.log({'Dis_Embedding_Norm': dct}, step=self.wandb_step)

            dct = {}
            for k in keys:
                dct[f'Class {k+1}'] = self.gen_emb_norms[k].item()
            wandb.log({'Gen_Embedding_Norm': dct}, step=self.wandb_step)
        ...
        if self.LOSS.apply_noisytwins:
            wandb.log({"noisytwins_loss": self.noisytwins_loss.item()}, step=self.wandb_step)
    ...
    def visualize_fake_images(self, num_cols, current_step):
        if self.global_rank == 0:
            self.logger.info("Visualize (num_rows x 8) fake image canvas.")
    ...
    # -----------------------------------------------------------------------------
    ## visualize fake images for long-tailed datasets for monitoring purpose 
    ## (Head 32 and Tail 32 classes).
    ## Also works for balanced datasets with head = tail, in reverse order as 
    ## Balanced = Special Case of Longtail).
    # -----------------------------------------------------------------------------
    def visualize_fake_images_lt(self, num_cols, current_step):
        if self.global_rank == 0:
            self.logger.info(f"Visualize (num_rows x {num_cols}) fake image canvas.")
        if self.gen_ctlr.standing_statistics:
            self.gen_ctlr.std_stat_counter += 1
        ...
        with torch.no_grad() if not requires_grad else misc.dummy_context_mgr() as ctx:
            ...
            if self.RUN.truncation_factor != -1:
                if self.is_stylegan:
                    assert 0 <= self.RUN.truncation_factor <= 1, "Stylegan truncation_factor must lie btw 0(strong truncation) ~ 1(no truncation)"
                else:
            ...
            if self.MODEL.visualize_real:
                ...
                for i in range(num_batches):
                    ...
                    real_labels = real_labels.to(self.local_rank)
            ...
            zs, fake_labels, _ = sample.sample_zy(z_prior=self.MODEL.z_prior,
                                                batch_size=32*num_cols, #self.OPTIMIZATION.batch_size,
                                                z_dim=self.MODEL.z_dim,
                                                num_classes=self.DATA.num_classes,
                                                truncation_factor=-1 if self.is_stylegan else self.RUN.truncation_factor,
                                                y_sampler=self.sampler,
                                                radius="N/A",
                                                device=self.local_rank)

            ## make fake_labels for tail and head classes manually instead of sampling randomly
            labels = torch.tensor(range(self.DATA.num_classes)).to(self.local_rank)
            counts = torch.tensor(self.train_dataloader.dataset.img_num_list).to(self.local_rank)

            ## j is the index to start picking classes, when in sorted order of their number of samples
            ## j=0 means the highest frequency class(Head), lowest freq class(Tail), and n is number of classes.
            j, n = 0, 32 #self.OPTIMIZATION.batch_size//num_cols
            ...
            for classes_type in  ["tail_", "head_"]:
                ...
                for idx in cls_indices:
                    fake_labels += [idx]*num_cols
                fake_labels = torch.tensor(fake_labels, dtype=torch.long).to(self.local_rank)
                
                if self.is_stylegan:
                    ws, fake_images = sample.stylegan_generate_images(zs=zs,
                                                            ...
                                                            generator_mapping=generator_mapping,
                                                            generator_synthesis=generator_synthesis,
                                                            truncation_psi=self.RUN.truncation_factor,
                                                            truncation_cutoff=self.RUN.truncation_cutoff,
                                                            mode="eval")
                ...
                if self.MODEL.visualize_real:
                    for i in range(cls_indices.cpu().shape[0]):
                        if i==0:
                            ...
                            ## Insert Black Images, if number of images of a class in dataset is less than num_cols in Canvas.
                            if real_canvas.shape[0]<num_cols:
                                real_canvas = torch.cat([real_canvas, torch.zeros((num_cols-N, C, H, W),dtype=real_canvas.dtype)], dim=0)
                        else:
                            real_imgs = real['images'][real['labels']==cls_indices[i].cpu()][:num_cols]
                            N, C, W, H = real_imgs.shape

                            ## Insert Black Images, if number of images of a class in dataset is less than num_cols in Canvas.
                            if real_imgs.shape[0]<num_cols:
                                real_imgs = torch.cat([real_imgs, torch.zeros((num_cols-N, C, H, W),dtype=real_imgs.dtype)], dim=0)
                            
                            real_canvas = torch.cat([real_canvas, real_imgs], dim=0)

                    misc.plot_img_canvas(images=real_canvas, 
                                        save_path=join(self.RUN.save_dir,
                                        "./figures/{dataset_name}_real/real_canvas_{classes_type}.png".format(dataset_name=self.DATA.name, 
                ...
                misc.plot_img_canvas(images=fake_images.detach().cpu(),
                                    save_path=join(self.RUN.save_dir,
                                    "figures/{run_name}/{classes_type}/generated_canvas_{step}.png".format(run_name=self.run_name, 
                                                                                                    classes_type= classes_type,step=current_step, ncols=num_cols)),
                                    num_cols=num_cols,
                                    logger=self.logger,
                                    logging=self.global_rank == 0 and self.logger)
                
                if self.RUN.train:
                    wandb.log({f"generated_images_{classes_type}": wandb.Image(fake_images)}, step=self.wandb_step)
    ...
    def evaluate(self, step, metrics, writing=True, training=False):
        ...
        ## while training is true, we evaluate only on the training set size defined on top 
        ## based on the datatset, but for final evaluation with training False, we evalauate on 
        ## 50k generated images always
    ...
    def run_tsne(self, dataloader):
        ...
        with torch.no_grad() if not requires_grad else misc.dummy_context_mgr() as ctx:
            ...
            for i in range(num_batches):
                ...
                real_dict = self.Dis(real_images, real_labels, is_real=True)
                if i == 0:
    ...
    # -----------------------------------------------------------------------------
    # visualize embeddings of tail classes in w-space using TSNE.
    # -----------------------------------------------------------------------------
    def run_w_space_tsne(self, dataloader):
        if self.global_rank == 0:
            self.logger.info("Start TSNE analysis in StyleGAN W-space using randomly sampled 10 classes.")
            self.logger.info("Use {ref} dataset and dataset_size//8 generated images for each class for visualization.".format(
                ref=self.RUN.ref_dataset))
        if self.gen_ctlr.standing_statistics:
            self.gen_ctlr.std_stat_counter += 1
        ...
        with torch.no_grad() if not requires_grad else misc.dummy_context_mgr() as ctx:
            ...
            fake = {}
            save_filepath = join(self.RUN.save_dir, "statistics", self.run_name, "w_space.npz")
            if os.path.isfile(save_filepath):
                print("File containing w_space embeddings found. Using that for tsne plots")
                loaded_file = np.load(save_filepath)
                fake["embeds"], fake["labels"] = loaded_file["embeds"], loaded_file["labels"]
            else:
                labels = torch.tensor(range(self.DATA.num_classes)).to(self.local_rank)
                counts = torch.tensor(self.train_dataloader.dataset.img_num_list).to(self.local_rank)
                j, n = 0, self.OPTIMIZATION.batch_size//8
                # _ , indices = torch.sort(counts, descending=True)
                # head_cls_indices = labels[indices[j:j+n]]

                _ , indices = torch.sort(counts, descending=False)
                tail_cls_indices = labels[indices[j:j+n]]
                cls_indices = tail_cls_indices.cpu().numpy()

                num_batches = len(dataloader.dataset) // self.OPTIMIZATION.batch_size
                for i in range(num_batches):
                    zs, _, _ = sample.sample_zy(z_prior=self.MODEL.z_prior,
                                                    batch_size=self.OPTIMIZATION.batch_size,
                                                    z_dim=self.MODEL.z_dim,
                                                    num_classes=self.DATA.num_classes,
                                                    truncation_factor=-1 if self.is_stylegan else self.RUN.truncation_factor,
                    ...
                    if self.is_stylegan:
                        ws, _ = sample.stylegan_generate_images(zs=zs,
                                                                fake_labels=fake_labels,
                                                                num_classes=self.DATA.num_classes,
                                                                style_mixing_p=0.0,
                                                                update_emas=False,
                    ...
                    if i == 0:
                        fake["embeds"] = ws[:,0,:].detach().cpu().numpy()
                        fake["labels"] = fake_labels.detach().cpu().numpy()
                    else:
                        fake["embeds"] = np.concatenate([fake["embeds"], ws[:,0,:].cpu().detach().numpy()],
                                                        axis=0)
                        fake["labels"] = np.concatenate([fake["labels"], fake_labels.detach().cpu().numpy()])
                
                ## Randomly picking 10 classes to visualize.
                if len(cls_indices) > 10:
                    cls_indices = np.random.choice(cls_indices, 10, replace=False)
                    fake["embeds"] = fake["embeds"][np.isin(fake["labels"], cls_indices)]
                    fake["labels"] = fake["labels"][np.isin(fake["labels"], cls_indices)]

                np.savez(save_filepath, **fake)
    ...
    # -----------------------------------------------------------------------------
    def calculate_intra_class_fid(self, dataset, ref_data):
        if self.global_rank == 0:
            self.logger.info("Start calculating iFID (use {num} fake images per class and {ref_data} images as the reference).".\
                             format(num=self.num_eval["ifid_num"], ref_data = ref_data))
        ...
        with torch.no_grad() if not requires_grad else misc.dummy_context_mgr() as ctx:
            ...
            for c in tqdm(range(self.DATA.num_classes)):
                ...
                c_fake_feats, _,_ = features.generate_images_and_stack_features(
                                                    ...
                                                    discriminator=self.Dis,
                                                    eval_model=self.eval_model,
                                                    num_generate=self.num_eval["ifid_num"],
                                                    y_sampler=c,
                                                    batch_size=self.OPTIMIZATION.batch_size,
                                                    z_prior=self.MODEL.z_prior,
                                                    truncation_factor=self.RUN.truncation_factor,
                ...
                metric_dict = {"iFID": fids}

                self.logger.info(f"Class {c} iFID score: {ifid_score}")
                save_dict = misc.accm_values_convert_dict(list_dict={"iFID": []},
                                                          value_dict=metric_dict,
                                                          step=c,
                                                          interval=1)
                misc.save_dict_npy(directory=join(self.RUN.save_dir, "statistics", self.run_name),
                                   name="iFID_{}".format(self.RUN.eval_backbone),
                                   dictionary=save_dict)

        if self.global_rank == 0 and self.logger:
            mean_iFID = sum(fids, 0.0) / len(fids)
            self.logger.info("Average iFID score: {}".format(mean_iFID))
            var_iFID = sum([ (x-mean_iFID)**2 for x in fids], 0.0) / len(fids)
            self.logger.info("Std iFID score: {}".format(var_iFID**0.5))
    ...
    def compute_GAN_train_or_test_classifier_accuracy_score(self, GAN_train=False, GAN_test=False):
        ...
        if self.RUN.ckpt_dir is not None and self.RUN.resume_classifier_train:
            is_pre_trained_model, phase = ckpt.check_is_pre_trained_model(ckpt_dir=self.RUN.ckpt_dir,
                                                                         GAN_train=GAN_train,
                                                                         GAN_test=GAN_test)
        ...
        for current_epoch in tqdm(range(epoch_trained, cas_setting["epochs"])):
            ...
            if is_best:
                ...
                misc.save_model_c(states, phase, self.RUN)
