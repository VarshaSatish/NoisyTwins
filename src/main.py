...
def load_configs_initialize_training():
    ...
    parser.add_argument("-umap", "--vis_emb_umap", action="store_true", help="visualize the umap of stylegan clss embeddings")
    ...
    parser.add_argument('--eval_backbone', type=str, default='Inception_V3', help="[SwAV, Inception_V3, CLIP]")
    ...
    if cfgs.RUN.load_train_hdf5:
        ...
        if cfgs.RUN.ref_dataset != "train":     
            ...
            hdf5_path_eval, crop_long_edge, resize_size = hdf5.make_hdf5(name=cfgs.DATA.name,
                                                                    ...
                                                                    DATA=cfgs.DATA,
                                                                    RUN=cfgs.RUN,
                                                                    split_type=cfgs.RUN.ref_dataset)
        else: hdf5_path_eval = hdf5_path_train
    else:
    ...
    return cfgs, gpus_per_node, run_name, hdf5_path_train, hdf5_path_eval, rank
