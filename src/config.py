...
class Configurations(object):
    ...
    def load_base_cfgs(self):
        ...
        # balancing hyperparameter for conditional image generation, d_cond_mtd in ["AC", "2C", "D2DCE"]
        ...
        # apply NoisyTwins regularization on generator
        self.LOSS.apply_noisytwins = False
        # step_count to start applying noisytwins in w space
        self.LOSS.noisytwins_start_iter = "N/A" 
        # strength of noisytwins regularization
        self.LOSS.noisytwins_lambda = "N/A"
        # noisytwins_momentum_lambda = weight to apply to off_diagonal terms in noisytwins
        self.LOSS.noisytwins_momentum_lambda = "N/A"
        ...
        # channel configuration for generator and discriminator 
        self.STYLEGAN.channel_base = None
        self.STYLEGAN.channel_max = None
    ...
    def define_augments(self, device):
        ...
        if self.AUG.apply_diffaug:
            ...
            if self.AUG.diffaug_type == "cr":
                ...
                #print(self.AUG.diffaug_type, simclr_aug.SimclrAugment)
                self.AUG.series_augment = simclr_aug.SimclrAugment(aug_type=self.AUG.diffaug_type).train().to(device).requires_grad_(False)
    ...
    def check_compatability(self):
        ...
        if self.MODEL.d_cond_mtd in ["AC", "D2DCE"]:
            assert self.MODEL.d_embed_dim != "N/A" and self.LOSS.m_p != "N/A" and self.LOSS.temperature != "N/A", \
                "When using AC or D2DCE, d_embed_dim, positive margin(m_p) and temperature must be specified."  
